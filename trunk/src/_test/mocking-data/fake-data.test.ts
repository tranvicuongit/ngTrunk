            ...
            if (environment.mockingData) {
                ...
                if (
                    ...
                    if (
                        ...
                        connection.mockRespond(
                            new Response(new ResponseOptions({ status: 200, body: users }))
                        );
                    } else {
                ...
                // get user by id
                if (
                    connection.request.url.match(/\/api\/users\/\d+$/) &&
                    connection.request.method === RequestMethod.Get
                ) {
                    ...
                    if (
                        ...
                        connection.mockRespond(
                            new Response(new ResponseOptions({ status: 200, body: user }))
                        );
                    } else {
                        // return 401 not authorised if token is null or invalid
                        connection.mockRespond(
                ...
                if (
                    ...
                    if (duplicateUser) {
                        return connection.mockError(
                            new Error('Email "' + newUser.email + '" is already registered')
                        );
                    }
                ...
                // token verify
                if (
                    connection.request.url.endsWith("/api/verify") &&
                    connection.request.method === RequestMethod.Get
                ) {
                    // check for fake auth token in header and return users if valid, this security
                    // is implemented server side in a real application
                    if (
                        ...
                        connection.mockRespond(
                            new Response(
                                new ResponseOptions({ status: 200, body: { status: "ok" } })
                            )
                        );
                    } else {
                        // return 401 not authorised if token is null or invalid
                ...
                // forgot password
                if (
                    connection.request.url.endsWith("/api/forgot-password") &&
                    connection.request.method === RequestMethod.Post
                ) {
                    // get parameters from post request
                    let params = JSON.parse(connection.request.getBody());
